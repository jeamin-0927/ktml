...
// src/ktml/decode.ts
var newElements = [...elements_default].sort((a, b) => b[1].length - a[1].length);
var newGlobal = [...gloabl_default].sort((a, b) => b[1].length - a[1].length);
var newAttributes = [...attributes_default].sort((a, b) => b[1].length - a[1].length);
var decodeFile = (content) => {
  for (const [htmlGlobal, ktmlGlobal] of newGlobal) {
    content = content.replace(new RegExp(ktmlGlobal, "g"), htmlGlobal);
  }
  for (const [htmlElements, ktmlElements] of newElements) {
    content = content.replace(new RegExp(`<${ktmlElements}`, "g"), `<${htmlElements}`);
    content = content.replace(new RegExp(`</${ktmlElements}`, "g"), `</${htmlElements}`);
  }
  ;
  for (const [htmlAttributes, ktmlAttributes] of newAttributes) {
    content = content.replace(new RegExp(`<([^>]*)${ktmlAttributes}([^>]*)>`, "g"), `<$1${htmlAttributes}$2>`);
  }
  return content;
};

...
// src/ktml/data/attributes/br.ts
var data14 = {
  en: "br",
  kr: "\uC904\uBC14\uAFC8",
  attributes: []
};
var br_default = data14;

// src/ktml/data/attributes/button.ts
var data15 = {
  en: "button",
  kr: "\uBC84\uD2BC",
  attributes: [
...
var caption_default = data17;

// src/ktml/data/attributes/cite.ts
var data18 = {
  en: "cite",
  kr: "\uC778\uC6A9",
  attributes: []
};
var cite_default = data18;

// src/ktml/data/attributes/code.ts
var data19 = {
  en: "code",
  kr: "\uCF54\uB4DC",
  attributes: []
};
var code_default = data19;
  ...
  for (const element of elementsList) {
    ...
    for (let i = 0; i < matches.length; i++) {
      ...
      for (const attribute of element.attributes.sort((a, b) => b.kr.length - a.kr.length)) {
        const pattern2 = new RegExp(`${attribute.kr}="([^"]*)"`, "g");
        const originMatchesAttribute = matches[i].match(pattern2);
        const matchesAttribute = matches[i].match(pattern2);
        if (!originMatchesAttribute || !matchesAttribute)
          continue;
        for (let j = 0; j < originMatchesAttribute.length; j++) {
          const regex2 = new RegExp(`${attribute.kr}`, "g");
          matchesAttribute[j] = originMatchesAttribute[j].replace(regex2, `${attribute.en}`);
          for (const child of attribute.children) {
            const regex3 = new RegExp(`"${child.kr}"`, "g");
            matchesAttribute[j] = matchesAttribute[j].replace(regex3, `"${child.en}"`);
          }
...
(async () => {
  ...
  if (process.argv[3]) {
    ...
    type = process.argv[2] === "encode" ? encodes : process.argv[2] === "decode" ? decodes : null;
    if (type)
      await type(dir);
  } else {
